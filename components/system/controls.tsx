...
export function StatusBanner({
  ...
  return (
    <output className={`block rounded-xl border px-4 py-3 text-sm ${colors}`}>
      {children}
    </output>
  );
}
...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-end bg-[#020711]/80 p-0 backdrop-blur-sm sm:place-items-center sm:p-6">
      <dialog
        open
        aria-modal="true"
        aria-labelledby="modal-title"
        className={`relative m-0 max-h-[92vh] w-full overflow-auto rounded-t-3xl border bg-[#0b1929] p-0 text-inherit shadow-2xl sm:max-w-2xl sm:rounded-3xl ${danger ? 'border-rose-400/25' : 'border-white/10'}`}
      >
