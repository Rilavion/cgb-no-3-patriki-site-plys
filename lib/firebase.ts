...
function friendlyError(error: unknown): Error {
  ...
  if (code.includes('permission-denied'))
    return new Error(
      'Firestore отклонил запрос. Проверьте Security Rules и App Check.',
    );
  if (code.includes('unavailable'))
    return new Error(
      'Firebase сейчас недоступен. Проверьте интернет и повторите попытку.',
    );
  ...
  return error instanceof Error
    ? error
    : new Error('Неизвестная ошибка Firebase.');
}

function requireDb() {
  if (!db)
    throw new Error(
      'Firebase ещё не подключён. Заполните firebaseConfig в lib/firebase.ts.',
    );
  return db;
}
...
export function subscribeToSupplies(
  ...
  return onSnapshot(
    suppliesQuery,
    (snapshot) =>
      onData(snapshot.docs.map((item) => mapSupply(item.id, item.data()))),
...
export async function createSupply(draft: SupplyDraft) {
  try {
    const total = Object.values(draft.organizations).reduce(
      (sum, value) => sum + value,
      0,
    );
...
export async function updateSupply(
  ...
  try {
    ...
    await runTransaction(database, async (transaction) => {
      ...
      if (!snapshot.exists())
        throw new Error('Поставка уже удалена другим пользователем.');
